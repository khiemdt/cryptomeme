...
type ButtonColor = "red" | "green" | "black" | "white" | "main";
  ...
  (
    ...
    const renderClassBySize = (_size: ButtonSize) => {
      switch (_size) {
        case "sm":
          // height: 32px
          return "h-[32px] px-[8px] text-16-16 " + renderBorderRadius(_size);
        case "md":
          // height: 36px
          return "h-[36px] px-[16px] text-16-16 " + renderBorderRadius(_size);
        case "lg":
          // height: 40px
          return "h-[40px] px-[16px] text-16-16 " + renderBorderRadius(_size);
        case "xl":
          // height: 48px
          return "h-[48px] px-[24px] text-16-16 " + renderBorderRadius(_size);
        case "xxl":
          // height: 56px
          return "h-[56px] px-[24px] text-18-18 " + renderBorderRadius(_size);
        case "3xl":
          // height: 56px
          return "h-[64px] px-[24px] text-18-18 " + renderBorderRadius(_size);
        default:
          return "";
      }
    };
    ...
    const renderActiveClass = (
      ...
      if (_size === "sm" || _size === "md" || _size === "lg") {
        borderSizeAndRadius =
          "active:after:border-[1.5px] active:after:rounded-[10px]";
      } else if (_size === "xl" || _size === "3xl") {
        borderSizeAndRadius =
          "active:after:border-[2px] active:after:rounded-[16px]";
      } else {
        borderSizeAndRadius =
          "active:after:border-[2px] active:after:rounded-[20px]";
      }
    ...
    const renderClassByVariantAndColor = (
      ...
      switch (color) {
        case "red":
          if (_variant === "contained") {
            return props.disabled
              ? "text-white bg-red-4 hover:bg-red-5"
              : "text-white bg-red hover:bg-red-7";
          }
          if (_variant === "outlined") {
            return `bg-none text-red ${renderBorderOutlinedClass(
              _size
            )} border-red hover:bg-red-1`;
          }
          if (_variant === "text") {
        ...
        case "white":
          ...
          if (_variant === "outlined") {
            return `bg-none text-white ${renderBorderOutlinedClass(
              _size
            )} border-neutral-900`;
          }
          if (_variant === "text") {
        ...
        case "main":
          if (_variant === "contained") {
            return `bg-main-1 text-white hover:opacity-90`;
          }
          if (_variant === "outlined") {
            return `bg-none ${renderBorderOutlinedClass(
              _size
            )} border-main-1 text-main-1 hover:opacity-90 hover:disabled:opacity-50`;
          }
          if (_variant === "text") {
            return `bg-none text-main-1 hover:opacity-50`;
          }
          return "";
        // default color = black
        default:
    ...
    const colorLoading =
      variant === "outlined" && color === "black" ? "#000000" : "#FFFFFF";
    ...
    return (
      <button
        ...
        className={clsx(
          ...
          // renderActiveClass(size, variant, color),
